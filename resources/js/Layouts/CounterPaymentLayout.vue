...
import { studentDataForAutofill } from '@/Components/MockData.js'; 

// --- MOCK STUDENT DATA FOR AUTOFILL (Now Dynamic) ---
// This now references the data mapped from the transaction list.
...
// --- MODIFIED FUNCTION: Autofill student data based on Campus ID ---
const autofillStudentData = (newCampusId) => {
    if (newCampusId) {
        // Uses the studentMockData (which is now dynamically sourced)
...
// --- ADDED WATCHER: To trigger autofill when campusId changes ---
...
const generateQR = async () => {
    ...
    const data = {
        ...
        amount: parseFloat(amount.value), // Keep this to ensure it's a number for the API
...
const clearForm = () => {
    ...
    // Clear QR/Reference/Message outputs
    qrCodeUrl.value = '';
    referenceCode.value = '';
    closeMessage();
};
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="name" v-model="name" required 
                                        class="p-2 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150 bg-gray-50">
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" id="referenceCode" :value="referenceCode"  disabled
                                ...
                                <div class="flex items-start md:items-end justify-start ">
                                    <button
                                        type="button"
                                        @click="clearForm"
                                        class="bg-gray-500 hover:bg-red-600 text-white font-semibold py-2 px-6 rounded-lg shadow-md transition duration-150 transform hover:scale-[1.01] flex items-center mt-6 md:mt-0"
                                    >
